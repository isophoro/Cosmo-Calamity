...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		switch (SONG.song.toLowerCase())
		{
			...
			case 'bad-gateway':
				dialogue = CoolUtil.coolTextFile(Paths.txt('bad-gateway/badgateDialogue'));
			case 'not-found':
				dialogue = CoolUtil.coolTextFile(Paths.txt('not-found/notfoundDialogue'));
			case 'service-unavailable':
				dialogue = CoolUtil.coolTextFile(Paths.txt('service-unavailable/serviceDialogue'));
		}
		...
		else if (SONG.song.toLowerCase() == 'bad-gateway' || SONG.song.toLowerCase() == 'not-found')
		{
			defaultCamZoom = 0.9;
			curStage = 'planetyo';
			galacticBG = new FlxSprite(-250, -130);
				
			galacticBG.frames = Paths.getSparrowAtlas('background sprite resized');
		...
		else if (SONG.song.toLowerCase() ==  'service-unavailable')
		{
			defaultCamZoom = 0.9;
			curStage = 'gr';
			galacticBG = new FlxSprite(-250, -130);
			...
			bgto1 = new FlxSprite(-500, -500);	
			bgto1.frames = Paths.getSparrowAtlas('cutscene BG');
			bgto1.animation.addByPrefix('move', "Symbol 2", 24, true);
			bgto1.animation.play('move');
			bgto1.antialiasing = true;
			bgto1.scrollFactor.set(0.9, 0.9);
			bgto1.visible = false;
			bgto1.updateHitbox();
			bgto1.active = true;
			add(bgto1);
			
			bg1 = new FlxSprite(-270, -270);	
			bg1.frames = Paths.getSparrowAtlas('sunsetroad');
			bg1.animation.addByPrefix('move', "roadbg", 24, true);
			bg1.animation.play('move');
			...
			bg2 = new FlxSprite(-300, -130);	
			bg2.frames = Paths.getSparrowAtlas('neocity');
			bg2.animation.addByPrefix('move', "neo", 24, true);
			bg2.animation.play('move');
			...
			bg3 = new FlxSprite(-250, -150);	
			bg3.frames = Paths.getSparrowAtlas('arcadefloor');
			bg3.animation.addByPrefix('move', "a", 24, true);
			bg3.animation.play('move');
			...
			platform = new FlxSprite(300, 700).loadGraphic(Paths.image('space_platform'));
	...
	function yesIntro(?dialogueBox:DialogueBox):Void
	{
		var yukiWave:FlxSprite = new FlxSprite(85, 150);
		yukiWave.frames = Paths.getSparrowAtlas('Yukichi_Entrance_Cutscene');
		yukiWave.animation.addByPrefix('idle', 'yukichi waving', 24, false);
		yukiWave.updateHitbox();
		add(yukiWave);
		...
		camHUD.visible = false;
		dad.visible = false;
		yukiWave.animation.play('idle');
		camFollow.x += 0;
		camFollow.y += 0;
		FlxG.camera.focusOn(camFollow.getPosition());
		FlxTween.tween(FlxG.camera, {zoom: 0.9}, 0.9);
		
		var blackScreen:FlxSprite = new FlxSprite(-2000, -200).makeGraphic(Std.int(FlxG.width * 3), Std.int(FlxG.height * 3), FlxColor.BLACK);
		add(blackScreen);
		blackScreen.scrollFactor.set();
		blackScreen.alpha = 0;
		...
		{
			FlxTween.tween(blackScreen, {alpha: 1}, 0.45, {
	...
	{
		var yukiSicko:FlxSprite = new FlxSprite(100, 100);
		yukiSicko.frames = Paths.getSparrowAtlas('Yukichi_Cutscenes');
		yukiSicko.animation.addByPrefix('idle', 'yukichi cutscene', 24, false);
		yukiSicko.updateHitbox();
		add(yukiSicko);
		...
		camHUD.visible = false;
		dad.visible = false;
		yukiSicko.animation.play('idle');
		camFollow.x += 0;
		camFollow.y += 0;
		FlxG.camera.focusOn(camFollow.getPosition());
		FlxTween.tween(FlxG.camera, {zoom: 0.9}, 0.9);
		FlxG.sound.play(Paths.sound('yukichi_sfx', 'shared'));
		new FlxTimer().start(2.5, function(tmr:FlxTimer)
		{
			camGame.shake((0.0099), (1.3));
	...
	function endSong():Void
	{
		...
		if (isStoryMode)
		{
			...
			if (storyPlaylist.length <= 0)
			{
				...
				if(curSong.toLowerCase() == 'service-unavailable')
					FlxG.switchState(new EndingState(totalNotesHit / totalPlayed * 100));  
				else
	...
	override function beatHit()
	{
		...
		if (curSong == 'service-unavailable')
		{
			switch (curBeat)
			{
				case 1:
					remove(boyfriend);
					boyfriend = new Boyfriend(775, 447, 'bf-sweat');
					add(boyfriend);
					remove(boyfriend);
					boyfriend = new Boyfriend(775, 447, 'bf');
					add(boyfriend);
					bg1.visible = true;
					bg2.visible = true;
					bg3.visible = true;
					...
					bg3.visible = false;
					platform.visible = false;
					warning = new FlxText(0,healthBarBG.y - 60, 0, "yo this level has some flashing lights", 24);
					warning.screenCenter(X);
					warning.setFormat(Paths.font("vcr.ttf"), 24, FlxColor.WHITE, CENTER, OUTLINE_FAST, FlxColor.BLACK);
				...
				case 3:
					...
					bg3.visible = false;
				case 5:
					FlxG.sound.play(Paths.sound('transition', 'shared'));
					bgto1.visible = true;
				case 8:
					bgto1.visible = false;
					bg1.visible = true;
					...
					boyfriend = new Boyfriend(775, 447, 'bf');
					add(boyfriend);
					FlxTween.tween(warning, {alpha: 0}, 0.32, { type: ONESHOT, 
						onComplete: function(twn:FlxTween){warning.destroy();}});				
				case 15:
					if(!anti_seizure){
						bg1.visible = false;
						bg2.visible = true;
		...
		if (!boyfriend.animation.curAnim.name.startsWith("sing") && !boyfriend.animation.curAnim.name.startsWith("sigh"))
		{
			boyfriend.playAnim('idle');
		}
